...
for x in {0..1}; do
  mkdir -m 0777 -p "data/origin/$x/data"
...
for x in {0..2047}; do y="$RANDOM"; for z in {0..3}; do
  echo "[ Url=\"pelican://discovery:8444/public/data/0.$((RANDOM / 32))\"; LocalFileName=\"/dev/null\" ]" >>"data/ads/input/$x.$y.exist"
done; done
...
for x in {0..2047}; do y="$RANDOM"; for z in {0..1}; do
  echo "[ Url=\"pelican://discovery:8444/private/data/0.$((RANDOM / 64))\"; LocalFileName=\"/dev/null\" ]" >>"data/ads/input/$x.$y.token"
done; done
...
for x in {0..2047}; do y="$RANDOM"; for z in {0..0}; do
  echo "[ Url=\"pelican://discovery:8444/public/data/9.$((RANDOM / 128))\"; LocalFileName=\"/dev/null\" ]" >>"data/ads/input/$x.$y.dne"
done; done
